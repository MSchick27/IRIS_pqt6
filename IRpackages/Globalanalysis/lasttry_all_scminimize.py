...
def calculate_DAS(A, C,parameters_forLMFIT):
    ...
    """
    # Initial guess for S
    Sparameters = parameters_forLMFIT[statenumber-1:]
    S_optimized = np.reshape(np.array(Sparameters),(C.shape[1], A.shape[1]))
...
def generate_kinetic_model_parameters(model_matrix):
    ...
    #generate rate k prameters for fitting 
...
def FIT(data,y0,t,parameters_forLMFIT,model_matrix):
    global result
    result = sc.optimize.minimize(OBJECTIVE_FIT,parameters_forLMFIT,args=(data,y0,t,model_matrix),options={'maxiter':300})#,tol=1e-9)
