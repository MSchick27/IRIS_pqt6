...
def kinetic_derivatives(y,t,params_LMFIT,model_matrix):
    ...
    for i in range(statenumber):
        ...
        for j in range(statenumber):
            if model_matrix[i,j]!=0:
                derivative = derivative - 1/params_LMFIT[str('k_'+str(statestring[i])+str(statestring[j]))] * y[i]
            if model_matrix[j,i]!=0:
                 derivative = derivative + 1/params_LMFIT[str('k_'+str(statestring[j])+str(statestring[i]))] * y[j]
...
def plot():
    ...
    k_param_number = i
    for i in range(k_param_number):
        parameter_evaluation.plot(np.arange(len(objectivetracking)),kparameter[i::k_param_number])
...
data =          np.load('Setupfiles/Testdata/waterbg_A_matrix.npy')
